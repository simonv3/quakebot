...
Promises.getBoundingBoxFromGoogle = function(user) {
  return new Promise(function(resolve, reject) {
    ...
    request(url, function (error, response, body) {
      if (!error && response.statusCode === 200) {
        ...
        if (results !== undefined) {
          ...
          resolve([northeast, southwest, northwest, southeast])
        }
        reject(error);
      }
    });
...
Promises.getUserBoundingBox = function(user) {
  return new Promise(function(resolve, reject) {
    ...
    makingARequest ++;
    console.log('makingARequest get user bounding box', makingARequest);
